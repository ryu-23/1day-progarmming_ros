    ...
    ui(new Ui::Widget)
{
    ...
    connect(ui->INPUT_SEND_BUTTEN,SIGNAL(clicked()),this,SLOT(initarray()));
...
void Widget::readData()
{
  ...
  //  qDebug() <<"\n"<<hexArray[2];
    ...
    for(int i= 0; i < 8 ; i++)
    {
        if(Binary[i]==1)    frames[i]->setStyleSheet("background-color: blue;");
        else if(Binary[i]==0) frames[i]->setStyleSheet("background-color: red;");
    }
