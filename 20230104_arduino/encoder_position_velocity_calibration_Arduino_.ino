...
float distance_puls = 1.222;
float distance_dack = 1.122;
float tt= 0;
int data;
int data2;
...
int pos_pid_control(void){
  ...
  pos_pid_pwm = (abs(pos_pid_pwm) > abs(new_velocity)) ? new_velocity : pos_pid_pwm;// 90% = new_velocity , 10% = pos_pid_pwm
...
void loop() {
  ...
  if (Serial.available() > 0) 
  {
     ...
     if (select_option == 'p') 
    {
      ...
      if(data>0)
        {
          target_Pos = distance_puls*data;
        }
        else if(data < 0)
        {
          target_Pos =distance_dack*data;
        }
      //target_angular_speed = data;
      //target_Pos = distance_puls*data;
      //tt = data;
      //target_Pos = tt*40*3.14/60;
     ...
     if (select_option == 'm') 
    {
      ...
      for (int i = 0; i < 4; i++) 
      {
        data_buffer2[i] = Serial.read();
      }
      
      Serial.write(data_buffer2[0]);
      Serial.write(data_buffer2[1]);
      Serial.write(data_buffer2[2]);
      Serial.write(data_buffer2[3]);
      s2=String(data_buffer2);
      data2 = s2.toInt();
      if(motor_direction =='r') {
        data2 = -data2;
      }
  ...
  Serial.print(encoderPos/(1.222*10));
  Serial.print(" cm");
