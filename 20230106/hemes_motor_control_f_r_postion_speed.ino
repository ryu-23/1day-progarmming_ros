...
volatile int32_t encoder2count_old = 0;

int f_speed = 0, r_speed = 0;
///////////////deadzoen///////////////////////
int dead_zone_front_positive = 20; //18
int dead_zone_front_negative = 26; //24
int dead_zone_rear_positive = 22;  //20
int dead_zone_rear_negative = 18;  //16
...
int pos_pid_control(void){
  int front_pos_pid_pwm = 0;
  int rear_pos_pid_pwm = 0;
...
int inverse_speed_mode(void){
  float pwm_inverse = 0;
  pwm_inverse = target_angular_speed * 2.0;
  return pwm_inverse;
}
...
//  output = !output;
  ...
  pos_error_sum = (pos_error_sum > 100) ? 100 : pos_error_sum;
  pos_error_sum = (pos_error_sum < -100) ? -100: pos_error_sum;
  if(fabs(pos_error)<=5) pos_error_sum = 0;

  if(target_angular_speed == 0 && target_Pos != 0){
  pos_pid_control();
  }
/////////////angular speed control///////////////////////
  angular_speed = encoder2count - encoder2count_old;
  ...
  encoder2count_old = encoder2count;
...
int angular_speed_pid_control(void){
  ...
  int front_angular_speed_pwm;
  int rear_angular_speed_pwm;
  ...
  front_angular_speed_pwm = dead_zone_front_correction(angular_speed_pid_pwm1);
  rear_angular_speed_pwm = dead_zone_rear_correction(angular_speed_pid_pwm1);    
  front_angular_speed_pwm = inverse_speed_mode();
  rear_angular_speed_pwm = inverse_speed_mode();
...
void R_MOTOR_CONTROL(int motor1_pwm)
{
...
void F_MOTOR_CONTROL(int MOTOR2_pwm)
{
...
void motor_control(int set_speed_front, int set_speed_rear)
{
  F_MOTOR_CONTROL(set_speed_front);
  R_MOTOR_CONTROL(set_speed_rear);  
}

int dead_zone_front_correction(int input_pwm){
  int pwm_out;

  if(input_pwm > 5)
  {
    pwm_out = input_pwm + dead_zone_front_positive;
  }
  else if(input_pwm < -5)
  {
    pwm_out = input_pwm - dead_zone_front_negative;
  }
